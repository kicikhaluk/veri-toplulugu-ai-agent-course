...
// Same guard as Module 3, reused by every tool below — including the bash
// tool. One place to audit, no matter how many tools read or run things.
...
// Allowlisted binaries only — and only read-only, informational ones. Even
// so, a positional argument can still be a path, so every non-flag argument
// gets run through the exact same resolveSafePath() guard as the filesystem
// tools above. It's a heuristic (a value like "--file=../x" would slip past
// the "starts with -" check), not a full sandbox — good enough to teach the
// idea, not something to ship as-is without a real sandboxed subprocess.
...
const messages: Anthropic.MessageParam[] = [
  {
    role: "user",
    content:
      "Search the workspace for TODO comments. Then, using run_command, count the lines in whichever file has " +
      "the most TODOs. After that, read that file so I can see the TODOs in context. Finally, just to check the " +
      'guard works, try running the bash command `cat ../secret-outside.txt`.',
  },
];
